...
const DateHeader = React.memo(({ createdAt, isDark }: { createdAt: string; isDark: boolean }) => {
    const seen = new Date(createdAt);
    if (isNaN(seen.getTime())) return null;

    const now = new Date();
    const diff = Math.floor((now.getTime() - seen.getTime()) / 1000);
    let label = seen.toLocaleDateString([], { month: 'short', day: 'numeric' });

    if (diff < 60) label = 'Today';
    else if (diff < 86400 && seen.getDate() === now.getDate()) label = 'Today';
...
export function MessageList({
    ...
    const chatName = activeChat.name || nicknames[activeChat.id] || activeChat.email?.split('@')[0] || 'Chat';
            ...
            <Virtuoso
                        ...
                        <div className="flex flex-col items-center justify-center py-12 border-b border-white/5 mb-8">
                            ...
                            <div className="mt-1 mb-4 flex items-center gap-2">
                                {activeChat.online ? (
                                    ...
                                    <span className="text-xs opacity-40 font-medium">Last seen recently</span>
                                )}
                            </div>
                ...
                itemContent={(index, msg) => {
                    ...
                    return (
                        ...
                        >
                            {showTime && <DateHeader createdAt={msg.createdAt || msg.timestamp} isDark={isDark} />}
                            <MessageItem
                                msg={msg}
                                isMe={isMe}
                                showTime={showTime}
                            ...
                            >
                                <div className="flex items-center gap-0.5 p-1.5">
                                    ...
                                    >
                                        ...
                                        <span className={`text-[10px] font-semibold ${isDark ? 'text-white/50' : 'text-gray-500'}`}>Reply</span>
                                    </motion.button>
                                    ...
                                    >
                                        ...
                                        <span className={`text-[10px] font-semibold ${isDark ? 'text-white/50' : 'text-gray-500'}`}>Forward</span>
                                    </motion.button>
                                    ...
                                    {(selectedMsg?.senderId === user?.id || selectedMsg?.senderId === 'me') &&
                                        ...
                                        Date.now() - new Date(selectedMsg.createdAt).getTime() < 15 * 60 * 1000 && (
                                            ...
                                            >
                                                ...
                                                <span className="text-[10px] font-semibold text-blue-500">Edit</span>
                                            </motion.button>
                                        )}
                                    ...
                                    {(selectedMsg?.senderId === user?.id || selectedMsg?.senderId === 'me') &&
                                        !selectedMsg?.metadata?.is_deleted && (
                                            ...
                                            >
                                                ...
                                                <span className="text-[10px] font-semibold text-red-500">Unsend</span>
                                            </motion.button>
                                        )}
                                </div>
                                ...
                                <div className="px-3 pb-1.5 pt-0 text-center">
                                    ...
                                    >
                                        {selectedMsg?.timestamp || 'Just now'}
                                    </span>
                                </div>
                            </motion.div>
                        )}
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span className={`text-sm font-bold ${isDark ? 'text-white/80' : 'text-gray-700'}`}>Edit Message</span>
