    ...
    if (!inView || !safeSrc) {
        return (
            <div ref={containerRef} className="py-1 flex items-center gap-2 opacity-50 min-w-[150px] h-10 animate-pulse">
                ...
                <span className="text-xs">Decrypting audio...</span>
            </div>
        );
    }
    return (
    ...
    if (isUnsent) {
        ...
        return (
            <div className={`relative text-[13px] italic opacity-40 select-none px-4 py-2 border border-dashed ${radiusAll} ${
                ...
                <span>🗑️ {isMe ? 'You deleted this message' : 'This message was deleted'}</span>
                {children}
            </div>
        );
    }
                            ...
                            <div className="flex-1 min-w-0">
                                <p dir="auto" className="font-bold opacity-90 mb-0.5 whitespace-nowrap overflow-hidden text-ellipsis text-[11px]">
                                    {(() => {
                                        ...
                                        return original?.senderId === 'me' || original?.senderId === user?.id ? 'You' : chatName;
                                    })()}
                                </p>
                                <p dir="auto" className="whitespace-nowrap overflow-hidden text-ellipsis opacity-70 text-[12px]">
                                    {(() => {
                                        const original = currentMessages.find((m) => m.id === replyToId);
                                        if (!original || !original.encryptedContent) return 'Deleted message';
                                        if (original.encryptedContent.startsWith('[IMAGE]') || original.encryptedContent.includes('E2EE_MEDIA:v1:')) return '📷 Photo';
                                        if (original.encryptedContent.startsWith('[AUDIO]')) return '🎤 Voice message';
