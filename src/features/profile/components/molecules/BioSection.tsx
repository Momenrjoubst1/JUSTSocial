...
export const BioSection: React.FC<BioSectionProps> = ({ bio: initialBio, isOwnProfile, onSave }) => {
  ...
  return (
    <div className="bg-card border border-border/40 shadow-sm rounded-2xl p-6 sm:p-8">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold text-foreground tracking-tight">About</h3>
        {isOwnProfile && !isEditing && (
          <button onClick={() => setIsEditing(true)} className="p-2 text-muted-foreground hover:bg-muted hover:text-foreground rounded-full transition-colors">
            <Edit size={18} />
          </button>
        )}
      </div>
      ...
      {!isEditing ? (
        <p className="text-muted-foreground leading-relaxed text-base whitespace-pre-wrap">
          {initialBio || "No bio available. Tell the world about yourself!"}
        </p>
      ) : (
