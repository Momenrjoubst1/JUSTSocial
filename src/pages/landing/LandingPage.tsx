...
interface RawCommunityMemberProfile {
  id: string;
...
import { ChevronDown, Bell, Star, Send, Lightbulb, Bug, CheckCircle2, ArrowRight, MessageSquare, CalendarDays, GraduationCap, Sparkles } from "lucide-react";
...
export function LandingPage({
  ...
  const [communityMembers, setCommunityMembers] = useState<CommunityMemberProfile[]>([]);
    ...
    const fetchCommunityMembers = async () => {
      ...
      try {
        const primaryResponse = await supabase
          .from("public_profiles")
          .select("id, full_name, username, avatar_url, bio, university, chat_hanger, created_at")
          .order("created_at", { ascending: false })
          .limit(12);
        ...
        if (queryError && (queryError.message?.includes("university") || queryError.message?.includes("chat_hanger"))) {
        ...
        const normalizedRows = rows
          ...
          .map((item) => {
            const displayName = item.full_name?.trim() || item.username?.trim() || "JUST Member";
            ...
            return {
              ...
              bio: item.bio?.trim() || "Passionate about learning and sharing skills with the community.",
        ...
        if (!isCancelled) {
          setMembersError("تعذر تحميل بيانات المستخدمين حالياً. حاول مرة أخرى بعد قليل.");
          setCommunityMembers([]);
        }
      } finally {
        if (!isCancelled) {
        ...
        <div className="max-w-7xl mx-auto w-full">
          ...
          >
            ...
            <p className="mt-4 text-[#A1A1A1] text-sm md:text-base max-w-2xl leading-relaxed">
              بطاقات حديثة تعرض المستخدمين الموجودين في الموقع مع أهم معلوماتهم بشكل واضح وسريع.
            </p>
          </motion.div>
                ...
                >
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-3 text-[11px] text-[#BABABA]">
                    <div className="flex items-center gap-2 rounded-xl border border-[#2D2D2D] bg-[#171717] px-3 py-2">
                      ...
                      <span className="truncate">{formatJoinedDate(member.createdAt)}</span>
                    </div>
                    <div className="flex items-center gap-2 rounded-xl border border-[#2D2D2D] bg-[#171717] px-3 py-2">
                      <GraduationCap className="w-3.5 h-3.5 text-[#D0D0D0]" />
                      <span className="truncate">
                        {member.university || member.chatHanger || "Skill Explorer"}
                      </span>
                    </div>
                  </div>
