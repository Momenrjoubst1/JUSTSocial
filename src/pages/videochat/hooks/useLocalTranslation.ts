...
export function useLocalTranslation(options: UseLocalTranslationOptions = {}) {
  ...
  const startTranslation = useCallback(async () => {
    ...
    if (!isConfigured) {
      setErrorMessage("Azure Speech key is missing. Set VITE_AZURE_SPEECH_KEY in your .env.");
      return;
    }
    ...
    try {
      ...
      recognizer.canceled = (_sender, event) => {
        if (event.reason === SpeechSDK.CancellationReason.Error) {
          setErrorMessage(event.errorDetails || "Translation was canceled by Azure Speech.");
        }
      ...
      setIsEnabled(false);
      setErrorMessage(error instanceof Error ? error.message : "Failed to start live translation.");
    } finally {
      setIsBusy(false);
    }
