      ...
      <div style={styles.statusBar}>
        <div>
          <span style={{ marginRight: '20px' }}>
            📊 Shapes: {whiteboardState.state?.shapes.size || 0}
          </span>
          <span style={{ marginRight: '20px' }}>
            🎯 Layers: {whiteboardState.state?.layers.length || 1}
          </span>
          {showStats && (
            <>
              <span style={{ marginRight: '20px' }}>
              ...
              <span>
                🔍 Zoom: {Math.round((whiteboardState.state?.zoom || 1) * 100)}%
              </span>
            </>
          )}
        </div>
        <div style={{ textAlign: 'right', fontSize: '11px', color: '#999' }}>
          {enableAI && '🤖 AI Enabled'}
          {enableCollaboration && ' • 👥 Collaboration Ready'}
        </div>
      </div>
    </div>
  );
