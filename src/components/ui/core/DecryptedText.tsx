...
export default function DecryptedText({
  ...
  const shuffleText = useCallback(
    (originalText: string, currentRevealed: Set<number>) => {
      return Array.from(originalText)
        .map((char, i) => {
          if (char === ' ') return ' ';
          if (currentRevealed.has(i)) return Array.from(originalText)[i];
  ...
  const fillAllIndices = useCallback((): Set<number> => {
    ...
    for (let i = 0; i < textLen; i++) s.add(i);
    return s;
  }, [text]);
  ...
  const triggerDecrypt = useCallback(() => {
    const textLen = Array.from(text).length;
    if (sequential) {
      orderRef.current = computeOrder(textLen);
  ...
  const triggerReverse = useCallback(() => {
    ...
    if (sequential) {
      ...
      orderRef.current = computeOrder(textLen).slice().reverse();
      pointerRef.current = 0;
      ...
      setRevealedIndices(prevRevealed => {
        if (sequential) {
          ...
          if (direction === 'forward') {
            if (prevRevealed.size < textLength) {
          ...
          if (direction === 'reverse') {
            ...
            const removeCount = Math.max(1, Math.ceil(textLength / Math.max(1, maxIterations)));
