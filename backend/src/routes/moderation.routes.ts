...
import { checkWithSightengine, type QueuedItem, type SightengineResponse } from '../services/moderation.service.js';
...
router.post('/report', reportLimiter, validate(reportSchema), asyncHandler(async (req: Request, res: Response) => {
  ...
  if (imageBase64 && process.env.SIGHTENGINE_API_USER) {
    try {
      ...
      if (sightRes.ok) {
        const result = (await sightRes.json()) as SightengineResponse;
...
router.get('/admin/moderation-queue', asyncHandler(async (req: Request, res: Response) => {
  ...
  const parsed = items.map((item: string): QueuedItem => JSON.parse(item) as QueuedItem);
...
router.delete('/admin/moderation-queue/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const toRemove = items.find(
    (item: string) => (JSON.parse(item) as QueuedItem).id === req.params.id
  );
