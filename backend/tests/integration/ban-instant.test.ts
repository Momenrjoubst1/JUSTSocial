...
import { supabase as supabaseAdmin } from '../../src/services/supabase.service.js';
...
describe.skipIf(!runSupabaseIntegration)('Ban Instant Enforcement', () => {
  ...
  it('يجب أن يرفض الطلب فوراً بعد تحديث banned_until', async () => {
    // 1. توليد توكن صالح
    const userRecord = await supabaseAdmin.auth.admin.getUserById(testUserId);
    const email = userRecord.data.user?.email;
    if (!email) throw new Error('Test user email was not found');
