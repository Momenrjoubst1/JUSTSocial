...
export function RemotePeerInfoOverlay({
  ...
  const { t: tVideochat } = useTranslation("videochat");
    ...
    >
      ...
      >
        <Avatar
          ...
          name={remoteUserInfo?.name || tCommon("user")}
          size={30}
      ...
      <div style={styles.peerInfo}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={styles.peerUsername}>
            {remoteUserInfo?.name || remotePeerIdentity || tCommon("guest")}
          </span>
      ...
      {remoteUserInfo?.userId && localUserInfo?.userId && remoteUserInfo.userId !== localUserInfo.userId && (
        <button
          ...
          title={isFollowingRemote ? tVideochat("unfollow") : tVideochat("addFriend")}
        >
