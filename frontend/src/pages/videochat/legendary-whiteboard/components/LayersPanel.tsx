  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <span style={styles.title}>{t("whiteboard.layers")}</span>
        <button
          onClick={() => {
            const name = prompt(
              t("whiteboard.layerNamePrompt"),
              t("whiteboard.newLayerDefault"),
            );
            if (name) onLayerCreate(name);
          }}
          style={styles.addButton}
          title={t("whiteboard.addNewLayer")}
        >
          +
        </button>
      </div>
              ...
              {renamingId === layer.id ? (
                ...
                <>
                  ...
                  <div
                    ...
                    title={t("whiteboard.shapesCount", { count: layer.shapes.length })}
                  />
                  ...
                  <div style={styles.layerControls}>
                    ...
                    <button
                      ...
                      title={layer.visible ? t("whiteboard.hideLayer") : t("whiteboard.showLayer")}
                    >
                      {layer.visible ? '👁️' : '🚫'}
                    </button>
                    ...
                    <button
                      ...
                      style={styles.iconButton}
                      title={layer.locked ? t("whiteboard.unlockLayer") : t("whiteboard.lockLayer")}
                    >
                      {layer.locked ? '🔒' : '🔓'}
                    </button>
                    ...
                    {layers.length > 1 && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (confirm(t("whiteboard.deleteLayerConfirm"))) {
                            onLayerDelete(layer.id);
                          }
                        }}
                        style={styles.iconButton}
                        title={t("whiteboard.deleteLayer")}
                      >
                        ✕
                      </button>
                    )}
