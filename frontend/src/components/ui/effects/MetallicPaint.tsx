...
vec2 fA(){
  vec2 c_pos=vP-.5;
  c_pos.x*=u_ratio>u_imgRatio?u_ratio/u_imgRatio:1.;
  c_pos.y*=u_ratio>u_imgRatio?1.:u_imgRatio/u_ratio;
  return vec2(c_pos.x+.5,.5-c_pos.y);
}
...
float mG(float hi,float lo,float t,float sh,float cv){
  sh*=(2.-u_sharp);
  float ci=smoothstep(.15,.85,cv),res=lo;
  float e1=.08/u_scale;
  res=mix(res,hi,smoothstep(0.,sh*1.5,t));
  res=mix(res,lo,smoothstep(e1-sh,e1+sh,t));
  float e2=e1+.05/u_scale*(1.-ci*.35);
  res=mix(res,hi,smoothstep(e2-sh,e2+sh,t));
  float e3=e2+.025/u_scale*(1.-ci*.45);
  res=mix(res,lo,smoothstep(e3-sh,e3+sh,t));
  float e4=e1+.1/u_scale;
  res=mix(res,hi,smoothstep(e4-sh,e4+sh,t));
  float rm=1.-e4,gT=clamp((t-e4)/rm,0.,1.);
  res=mix(res,mix(hi,lo,smoothstep(0.,1.,gT)),smoothstep(e4-sh*.5,e4+sh*.5,t));
  return res;
}
...
void main(){
  ...
  vec3 hi_col=u_lightColor*u_bright;
  vec3 lo_col=u_darkColor*(2.-u_bright);
  lo_col.b+=smoothstep(.6,1.4,sc.x+sc.y)*.08;
  vec2 fC=sc-.5;
  ...
  fl-=tb*sl*1.8*eI;
  float cA_val=cv*clamp(pow(sc.y,.12),.25,1.);
  fl*=.12+(1.05-lD)*cA_val;
  ...
  float rC=mG(hi_col.r,lo_col.r,rP,sf+.018+u_refract*cv*.025,cv);
  float gP=fract(fl);
  float gC=mG(hi_col.g,lo_col.g,gP,sf+.008/max(.01,1.-sl),cv);
  float bP=fract(fl-bO);
  float bC=mG(hi_col.b,lo_col.b,bP,sf+.008,cv);
...
function processImage(img: HTMLImageElement) {
  ...
  let currMax = 0;
  for (let i = 0; i < size; i++) if (u[i] > currMax) currMax = u[i];
  if (currMax === 0) currMax = 1;

  const outData = ctx.createImageData(width, height);
  for (let i = 0; i < size; i++) {
    const px = i * 4;
    const depth = u[i] / currMax;
...
export const MetallicPaint = ({
  ...
  distortion = 1,
  contour = 0.2,
  tintColor = '#ffffff', // Changed default from purple tint to white (neutral metal)
  className = "",
  style = {}
}: MetallicPaintProps) => {
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      if (!isVisibleRef.current) {
        lastTimeRef.current = time; // maintain time difference when paused
        return;
      }
  ...
  return <canvas ref={canvasRef} className={`paint-container ${className}`} style={style} />;
}
