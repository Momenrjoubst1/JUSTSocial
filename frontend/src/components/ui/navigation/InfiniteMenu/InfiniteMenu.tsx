...
function createProgram(gl: WebGL2RenderingContext, shaderSources: string[], _tfv?: any, attribLocations?: Record<string, number>) {
...
class InfiniteGridMenu {
  ...
  init(onInit: any) {
    const ctx = this.canvas.getContext('webgl2', { antialias: true, alpha: true });
    if (!ctx) {
      console.error('InfiniteMenu: WebGL 2 not available');
      return;
    }
    this.gl = ctx;
    const gl = this.gl;
      ...
      this.items.map(
        (item: any) =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              // Fallback: create a canvas with the flag emoji
              ...
              fctx.font = `${cellSize * 0.5}px serif`;
              fctx.textAlign = 'center';
              fctx.textBaseline = 'middle';
              fctx.fillText(item.emoji || '🌐', cellSize / 2, cellSize / 2 - 20);
              fctx.font = `bold ${cellSize * 0.08}px Inter, sans-serif`;
              fctx.fillStyle = '#fff';
              fctx.fillText(item.title || '', cellSize / 2, cellSize * 0.8);
  ...
  animate(deltaTime: number) {
    ...
    positions.forEach((p, ndx) => {
      ...
      mat4.multiply(matrix, matrix, mat4.targetTo(mat4.create(), [0, 0, 0], p as any, [0, 1, 0]));
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      const itemIndex = index % itemsRef.current.length;
      setActiveItem(itemsRef.current[itemIndex]);
    };
    ...
    const sketch = new InfiniteGridMenu(
      ...
      items,
      handleActiveItem,
      (moving: boolean) => setIsMoving(moving),
      (sk: any) => {
        sk.resize();
        sk.run();
      },
  ...
  // items is memoized in the parent and scale is a primitive, so this is stable
  }, [items, scale]);
