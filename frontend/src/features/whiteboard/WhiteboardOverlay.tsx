...
import {
  ...
  WHITEBOARD_MIN_ZOOM,
  WHITEBOARD_MAX_ZOOM,
  WHITEBOARD_GRID_SIZE,
  WHITEBOARD_LASER_LIFETIME,
  WHITEBOARD_TOOLS,
  createWhiteboardId,
} from "./constants";
...
export const WhiteboardOverlay: React.FC<WhiteboardOverlayProps> = ({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const filtered = prev.filter(
          (el) =>
            el.tool !== "laser" || now - (el.createdAt || 0) < WHITEBOARD_LASER_LIFETIME,
        );
      ...
      ctx.globalAlpha =
        el.tool === "highlighter"
          ...
          : el.tool === "laser"
            ? Math.max(
              0,
              1 - (Date.now() - (el.createdAt || Date.now())) / WHITEBOARD_LASER_LIFETIME,
            )
            : el.opacity;
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      const newZoom = Math.max(WHITEBOARD_MIN_ZOOM, Math.min(WHITEBOARD_MAX_ZOOM, zoom * factor));
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (!e.ctrlKey && !e.metaKey && !e.altKey) {
        const match = WHITEBOARD_TOOLS.find(
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ZOOM CONTROLS
     ══════════════════════════════════════════════════════════════════════ */
  const zoomIn = () => setZoom((z) => Math.min(WHITEBOARD_MAX_ZOOM, z * 1.25));
  const zoomOut = () => setZoom((z) => Math.max(WHITEBOARD_MIN_ZOOM, z / 1.25));
