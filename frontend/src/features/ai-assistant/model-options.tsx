...
const getModelIcon = (modelValue: string): ReactNode => {
  ...
  if (value === "gpt-4o" || value === "gpt-4o-mini") {
    return wrapIcon(<GitHubIcon className={iconClass} />);
  }

  if (value.startsWith("anthropic/") || value.includes("claude")) {
    return wrapIcon(<AnthropicIcon className={iconClass} />);
  }

  if (value.startsWith("google/") || value.includes("gemini") || value.includes("gemma")) {
    return wrapIcon(<GoogleGeminiIcon className={iconClass} />);
  }

  if (value.startsWith("xai/") || value.startsWith("x-ai/") || value.includes("grok")) {
    return wrapIcon(<XAIIcon className={iconClass} />);
  }

  if (value.startsWith("meta/") || value.startsWith("meta-llama/") || value.includes("llama")) {
    return wrapIcon(<MetaIcon className={iconClass} />);
  }

  if (value.includes("mixtral") || value.includes("mistral")) {
    return wrapIcon(
      <span className="inline-flex size-3.5 items-center justify-center text-[9px] font-bold leading-none text-orange-400">
        Mx
      </span>
    );
  }

  if (value.startsWith("alibaba/") || value.startsWith("qwen/") || value.includes("qwen")) {
