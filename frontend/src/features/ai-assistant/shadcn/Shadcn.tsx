...
import { ComposerQuotePreview, QuoteBlock, SelectionToolbar } from "../ui/quote";
...
import { LexicalComposerInput, type MentionChipProps } from "@assistant-ui/react-lexical";
import { useState, type FC } from "react";
import { Sheet, SheetContent, SheetTrigger } from "@/components/ui/sheet";
import { ModelSelector } from "../ui/model-selector";
import { docsModelOptions } from "../model-options";
...
const Logo: FC = () => {
  return (
    <div className="flex items-center gap-2 px-2 font-medium text-sm">
      ...
      <span className="text-foreground/90">JUST Social AI</span>
    </div>
  );
};
...
const ModelPicker: FC = () => {
  return <ModelSelector models={models} defaultValue={DEFAULT_MODEL_ID} variant="outline" />;
};
...
function DirectiveChip(props: MentionChipProps) {
  const { mentionId, mentionType, label } = props;
  const showWrench = mentionType !== "command";

  return (
    <span
      className="aui-directive-chip"
      data-directive-type={mentionType}
      data-directive-id={mentionId}
    >
      {showWrench && (
        ...
        <ComposerPrimitive.AttachmentDropzone asChild>
          ...
          >
            ...
            <ComposerPrimitive.Unstable_TriggerPopoverRoot
              ...
              onSelect={{
                type: "action",
                handler: slash.action.onExecute,
              }}
            >
              <LexicalComposerInput
                mentionChip={DirectiveChip}
                formatter={mention.directive.formatter}
...
const MessageError: FC = () => {
  return (
    <MessagePrimitive.Error>
      <ErrorPrimitive.Root className="aui-message-error-root mt-2 flex flex-col gap-2 rounded-md border border-destructive bg-destructive/10 p-3 text-destructive text-sm dark:bg-destructive/5 dark:text-red-200">
        <div className="font-medium">عذراً، واجهت مشكلة في معالجة طلبك.</div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Processing Indicator for empty running messages */}
        <AuiIf condition={(s) => s.message.content.length === 0 && s.thread.isRunning}>
          <div className="flex items-center gap-2 text-muted-foreground animate-pulse mt-2">
            <div className="flex gap-1">
              <span className="size-2 rounded-full bg-primary/60"></span>
              <span className="size-2 rounded-full bg-primary/60 delay-75"></span>
              <span className="size-2 rounded-full bg-primary/60 delay-150"></span>
            </div>
            <span className="text-sm">Sigma يعالج سؤالك ويبحث في المصادر...</span>
          </div>
        </AuiIf>
        <MessageError />
      </div>
  ...
  return (
    ...
    >
      ...
      <ActionBarPrimitive.Reload asChild>
        <TooltipIconButton tooltip="إعادة المحاولة">
          <RefreshCwIcon />
        </TooltipIconButton>
      </ActionBarPrimitive.Reload>
      <ActionBarPrimitive.FeedbackPositive asChild>
        <TooltipIconButton tooltip="إجابة مفيدة">
      ...
      <ActionBarPrimitive.FeedbackNegative asChild>
        <TooltipIconButton tooltip="إجابة غير مفيدة">
  ...
  return (
    ...
    >
      ...
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0">
        <div className="aui-user-message-content wrap-break-word peer rounded-2xl bg-muted px-4 py-2.5 text-foreground empty:hidden" dir="auto">
