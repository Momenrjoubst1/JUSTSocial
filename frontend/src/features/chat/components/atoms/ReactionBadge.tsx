...
export const ReactionBadge = React.memo(({ reactions, reactionType, reactionBy, isMe }: ReactionBadgeProps) => {
    ...
    const getInstantName = (id: string) => {
        if (id === user?.id) return tCommon('you');
        ...
        const fetchNames = async () => {
            ...
            try {
                ...
                if (isMounted && data && !error) {
                    ...
                    // add new fetched
                    data.forEach(u => {
                        const name = u.id === user?.id ? tCommon('you') : (u.full_name || u.username || tChat('someone'));
                        names[u.id] = name;
    ...
    const getTooltipText = () => {
        return Object.entries(activeReactions).map(([id, emoji]) => {
            const name = getInstantName(id) || displayNames[id] || (isLoading ? '...' : tChat('someone'));
            return `${name}: ${emoji}`;
        }).join('\n');
    };
