...
const EmojiPicker = lazy(() =>
    import('@/components/ui/EmojiPicker').then((module) => ({
...
export function ChatInput({ broadcastTyping }: { broadcastTyping: (typing: boolean) => void }) {
    ...
    const translationOptions = [
        { code: null, label: `🚫 ${t('translateOptions.off')}` },
        { code: 'auto-correct', label: `✨ ${t('translateOptions.autoCorrect')}` },
        { code: 'en-casual', label: `🇬🇧 ${t('translateOptions.enCasual')}` },
        { code: 'en-formal', label: `🇬🇧 ${t('translateOptions.enFormal')}` },
        { code: 'ar-formal', label: `🇦🇪 ${t('translateOptions.arFormal')}` },
        { code: 'ar-sa', label: `🇸🇦 ${t('translateOptions.arSaudi')}` },
        { code: 'ar-eg', label: `🇪🇬 ${t('translateOptions.arEgyptian')}` },
        { code: 'ar-lev', label: `🇱🇧 ${t('translateOptions.arLevantine')}` },
        { code: 'ar-gulf', label: `🇦🇪 ${t('translateOptions.arGulf')}` },
        { code: 'ar-ma', label: `🇲🇦 ${t('translateOptions.arMoroccan')}` },
        { code: 'fr-casual', label: `🇫🇷 ${t('translateOptions.frCasual')}` },
        { code: 'es-casual', label: `🇪🇸 ${t('translateOptions.esCasual')}` },
    ];
        ...
        return (
            <div className={`p-6 rounded-3xl border text-center backdrop-blur-md ${isDark ? 'bg-red-500/5 border-red-500/20' : 'bg-red-50 border-red-200'}`}>
                <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="font-bold text-red-500">{t('blockedUserTitle')}</p>
                    <p className="text-sm opacity-60">{t('blockedUserDescription')}</p>
                </div>
            </div>
        );
    }
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                            <p className="text-[13px] font-bold text-primary truncate leading-tight mb-0.5">
                                {replyingTo.senderId === 'me' || replyingTo.senderId === user?.id ? tCommon('you') : chatName}
                            </p>
                            <p className="text-[13px] opacity-60 truncate leading-tight">
                                {replyingTo.encryptedContent.startsWith('[IMAGE]') ? t('reply.photo') : replyingTo.encryptedContent.startsWith('[AUDIO]') ? t('reply.voiceMessage') : <EmojiText text={replyingTo.encryptedContent} size={14} />}
                            </p>
                        </div>
                                ...
                                >
                                    <div className="p-2 flex flex-col gap-0.5 max-h-64 overflow-y-auto custom-scrollbar">
                                        <div className="sticky top-0 z-10 pb-1 mb-1 border-b border-border/50 bg-inherit backdrop-blur-md">
                                            <p className="text-[11px] font-bold px-2 py-1 opacity-50 uppercase tracking-wider">{t("translateTo")}</p>
                                        </div>
                    ...
                    <div className="w-full relative">
                        ...
                        >
                            {inputText ? (
                                ...
                                <span className="opacity-50">{t("messagePlaceholder")}</span>
                            )}
                        </div>
