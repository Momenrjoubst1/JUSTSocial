...
export function useSendMessage() {
    ...
    const sendMessage = async (textPayload: string, replyId?: string, recipientId?: string, providedTempId?: string, mediaFile?: File) => {
        ...
        const lastMsgSnippet = isSystemAlert ? t('securityAlertSnippet') : isImage ? t('photoAttached') : isAudio ? t('voiceMessageSnippet') : finalPayload;
        ...
        setContacts(prev => prev.map(c =>
            c.id === targetRecipientId ? { ...c, lastMessage: lastMsgSnippet, time: t('now') } : c
        ));
        ...
        try {
            if (pLoad) {
                ...
                if (modResult && modResult.allowed === false) {
                    alert(t('moderation.blockedContent'));
                    setAllMessages(prev => {
        ...
        if (mediaFile && targetConversationId) {
            try {
                ...
                alert(t('moderation.uploadFailed'));
                return;
            }
        }
