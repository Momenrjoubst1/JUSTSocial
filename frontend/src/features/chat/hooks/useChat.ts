            ...
            try {
                        ...
                        if (data && !error && isMounted) {
                            const mapped: Contact[] = data.map(u => ({
                                id: u.id,
                                name: u.full_name || u.username || tCommon('user'),
                                ...
                                lastMessage: t('emptyState.startConversation'),
                                time: '',
                                unread: 0,
                                online: false,
                                typing: false,
        ...
        const fetchActiveMessages = async () => {
            ...
            if (data && !error && isMounted) {
                ...
                const formattedMsgs: Message[] = decryptedData.map((m) => {
                    const timeStr = new Date(m.created_at).toLocaleTimeString(i18n.language, { hour: '2-digit', minute: '2-digit' });
                    return {
                        id: m.id,
