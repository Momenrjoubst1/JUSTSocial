...
export function usePresence() {
    ...
    useEffect(() => {
        ...
        channel
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, async (payload) => {
                ...
                const timeStr = new Date(m.created_at || Date.now()).toLocaleTimeString(i18n.language, { hour: '2-digit', minute: '2-digit' });
                        ...
                        setContacts(prev => prev.map(c => {
                            if (c.id === otherId) {
                                ...
                                const snippet = decryptedText.startsWith('[IMAGE]') 
                                    ? (isMe ? `${tCommon('you')}: ${t('photo')}` : t('photo')) 
                                    : decryptedText.startsWith('[AUDIO]') 
                                    ? (isMe ? `${tCommon('you')}: ${t('voiceMessageSnippet')}` : t('voiceMessageSnippet')) 
                                    : decryptedText;
                        ...
                        setContacts(prev => prev.map(c => {
                            if (c.id === otherId) {
                                const snippet = isUnsent 
                                    ? t('deletedMessage') 
                                    : newDecryptedContent!.startsWith('[IMAGE]') ? t('photo') 
                                    : newDecryptedContent!.startsWith('[AUDIO]') ? t('voiceMessageSnippet') 
                                    : newDecryptedContent!;
    ...
    }, [user, keysReady, privateKey, setContacts, setAllMessages, i18n.language, t, tCommon]);
