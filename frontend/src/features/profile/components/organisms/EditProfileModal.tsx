...
import {
  HANGER_OPTIONS,
  HangerId,
} from "@/features/chat/components/atoms/ChatHanger";
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/20 dark:border-white/10 bg-white/40 dark:bg-black/40 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold tracking-tight text-foreground">
              {String(t("profile:editModal.title"))}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        >
          <div className="space-y-4">
            <label className="text-sm font-bold text-foreground flex items-center gap-2">
              ...
              {String(t("profile:editModal.avatarFrame"))}
            </label>
                  ...
                  <div className="w-full h-full rounded-xl overflow-hidden bg-muted flex flex-col items-center justify-center gap-1 text-center p-1">
                    {frame.id === "none" ? (
                      <div className="text-[10px] font-bold text-muted-foreground">
                        {String(t("profile:avatarFrames.none"))}
                      </div>
                    ) : (
                      <>
                        <div
                          ...
                          style={{
                            background:
                              frame.colors.length > 1
                                ? `linear-gradient(to top right, ${frame.colors.join(",")})`
                                : frame.colors[0],
                            borderColor:
                              frame.colors[0] === "transparent"
                                ? "rgba(0,0,0,0.1)"
                                : frame.colors[0],
                          }}
                        />
                        <span className="text-[9px] font-bold text-foreground truncate w-full">
                          {String(t(`profile:avatarFrames.${frame.id}`))}
                        </span>
                      </>
                    )}
                  </div>
          ...
          <div className="space-y-4 border-t border-white/10 pt-4 mt-4">
            <label className="text-sm font-bold text-foreground flex items-center gap-2">
              ...
              {String(t("profile:editModal.chatHanger"))}
            </label>
            <div className="flex flex-wrap gap-3">
              {HANGER_OPTIONS.map((hanger) => (
                <button
                  key={hanger.id}
                  type="button"
                  onClick={() => setChatHanger(hanger.id)}
                  className={`flex flex-col items-center justify-center gap-2 p-3 rounded-2xl transition-all duration-300 border-2 min-w-[70px] ${
                    chatHanger === hanger.id
                      ? "border-purple-500 bg-purple-500/10 shadow-lg scale-105"
                      : "border-white/20 bg-white/5 hover:bg-white/10 hover:border-white/40"
                  }`}
                >
                  <span className="text-2xl">{hanger.icon}</span>
                  <span className="text-[10px] font-bold">
                    {String(t(`profile:chatHangers.${hanger.id as HangerId}`))}
                  </span>
                </button>
              ))}
            </div>
            <p className="text-[10px] text-muted-foreground italic text-center">
              {String(t("profile:editModal.chatHangerHelp"))}
            </p>
          </div>

          <div className="space-y-2 group relative">
            <label className="text-sm font-bold text-foreground">
              {String(t("profile:editModal.usernameLabel"))}
            </label>
            <div className="relative">
              <input
                ...
                title={String(t("profile:editModal.usernameImmutable"))}
              />
              {showUsernameWarning && (
                <p className="absolute -bottom-5 left-0 text-[10px] font-medium text-red-500 animate-in fade-in slide-in-from-top-1 duration-200">
                  {String(t("profile:editModal.usernameWarning"))}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-foreground">
              {String(t("profile:editModal.fullNameLabel"))}
            </label>
            <input
              type="text"
              value={fullName}
              onChange={(e) => setFullName(e.target.value)}
              placeholder={String(t("profile:editModal.fullNamePlaceholder"))}
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-foreground">
              {String(t("profile:editModal.universityLabel"))}
            </label>
            <select
              value={university}
            ...
            >
              <option value="">{String(t("profile:editModal.universityEmpty"))}</option>
              {UNIVERSITY_OPTIONS.map((item) => (
                <option key={item} value={item}>
                  {item === UNIVERSITY_OPTIONS[0]
                    ? String(t("profile:universities.just"))
                    : item}
                </option>
              ))}
            </select>
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-foreground">
              {String(t("profile:editModal.bioLabel"))}
            </label>
            <textarea
              value={bio}
              onChange={(e) => setBio(e.target.value)}
              placeholder={String(t("profile:editModal.bioPlaceholder"))}
            ...
            <div className="text-right text-[10px] font-bold text-muted-foreground flex justify-end items-center gap-1">
              <span className={bio.length > 280 ? "text-orange-500" : ""}>
                {bio.length}
              </span>
              <span>/ 300</span>
            </div>
          ...
          >
            {isSaving ? (
              <>
                ...
                {String(t("profile:editModal.saving"))}
              </>
            ) : (
              String(t("profile:editModal.saveChanges"))
            )}
          </button>
        </div>
      </div>
